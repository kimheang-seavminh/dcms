...
<head>
    ...
    <title>DMS - Assigned_documents</title>
    <!-- Tailwind CSS CDN -->
...
<body class="flex h-screen overflow-hidden">
    ...
    // Fetch assigned documents from the database
    // Assuming 'assigned to you' means documents created by you for now.
    // In a full system, you'd have an 'assignments' table.
    $assigned_documents = [];
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="flex items-center justify-between p-6 bg-white shadow-md">
            <div class="flex items-center">
                ...
                <h1 class="text-2xl font-semibold text-gray-800"><?php echo $lang['assigned_documents']; ?></h1>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div class="flex flex-col md:flex-row md:items-end justify-between mb-4 space-y-4 md:space-y-0 md:space-x-4">
                    <div class="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label for="search-name" class="block text-xl font-medium text-gray-700 mb-2"><?php echo $lang['search by name'];?></label>
                            <input type="text" id="search-name" placeholder="Search by name or description"
                                   class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </div>
                        <div>
                            <label for="search-meta" class="block text-xl font-medium text-gray-700 mb-2"><?php echo $lang['search by meta tags'];?></label>
                            <input type="text" id="search-meta" placeholder="Search by meta tags"
                                   class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </div>
                        <div>
                            <label for="select-category" class="block text-xl font-medium text-gray-700 mb-2"><?php echo $lang['select_category'];?></label>
                    ...
                    <div class="flex space-x-3">
                        <a href="add_document.php" class="px-4 py-2 bg-indigo-600 text-white rounded-md shadow-sm hover:bg-indigo-700 transition duration-150 ease-in-out flex items-center">
                            <i class="fas fa-plus mr-2"></i> <?php echo $lang['add Document']; ?>
                        </a>
                        <button class="px-4 py-2 bg-red-500 text-white rounded-md shadow-sm hover:bg-red-600 transition duration-150 ease-in-out flex items-center">
                            <i class="fas fa-bell mr-2"></i> <?php echo $lang['my reminders']; ?>
                        </button>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-1xs font-medium text-gray-500 uppercase tracking-wider"><?php echo $lang['category name']; ?></th>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-1xs font-medium text-gray-500 uppercase tracking-wider text-red-600"><?php echo $lang['expired date']; ?> <i class="fas fa-sort-down ml-1"></i></th>
                                <th scope="col" class="px-6 py-3 text-left text-1xs font-medium text-gray-500 uppercase tracking-wider"><?php echo $lang['created by']; ?> <i class="fas fa-sort-down ml-1"></i></th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php if (empty($assigned_documents)): ?>
                                <tr>
                                    <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">No assigned documents found.</td>
                                </tr>
                            <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm <?php echo (!empty($document['expired_date']) && strtotime($document['expired_date']) < time()) ? 'text-red-500 font-semibold' : 'text-gray-500'; ?>">
                                        <?php echo htmlspecialchars($document['expired_date']); ?>
                                    </td>
