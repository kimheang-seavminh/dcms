...
// Check if the user has permission to manage users (e.g., Super Admin or specific user management role)
// For simplicity, let's assume only Super Admin can add users for now.
if ($user_role_name !== 'Super Admin') {
    header("Location: dashboard.php?message=Access Denied: You do not have permission to add users.&type=error");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_user'])) {
    ...
    if (empty($email) || empty($password) || empty($role_id)) {
        ...
        if ($check_stmt === false) {
            // Error preparing the statement for checking email existence
            $message = "Database error: Could not prepare email check query. Error: " . $conn->error;
            $message_type = 'error';
        } else {
            ...
            if ($check_result->num_rows > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO users (email, password, first_name, last_name, mobile_number, role_id) VALUES (?, ?, ?, ?, ?, ?)");
                if ($stmt === false) {
                    // Error preparing the statement for inserting user
                    $message = "Database error: Could not prepare user insertion query. Please check your 'users' table column names. Error: " . $conn->error;
                    $message_type = 'error';
                } else {
                    $stmt->bind_param("sssssi", $email, $hashed_password, $first_name, $last_name, $mobile_number, $role_id);

                    if ($stmt->execute()) {
                        $message = "User '" . htmlspecialchars($email) . "' added successfully.";
                        $message_type = 'success';
                        // Clear form fields after successful submission
                        $email = ''; $password = ''; $first_name = ''; $last_name = ''; $mobile_number = ''; $role_id = '';
                    } else {
...
if ($roles_result === false) {
    ...
    // This error won't stop the page from loading, but the dropdown will be empty.
